...
public extension TabNavigationBuilder {
    func build(item: LinkItem) -> [RouteViewController] {
        item.pathList.compactMap { path in
            routeBuilderList.first(where: { $0.matchPath == path })?.routeBuild(rootNavigator, item.encodedItemString, dependency, item.arguments)
        }
    }
    ...
    func pickBuild(item: LinkItem) -> RouteViewController? {
        routeBuilderList
            ...
            .routeBuild(rootNavigator, item.encodedItemString, dependency, item.arguments)
    }

    func firstPick(controller: UINavigationController?, item: LinkItem)
        -> RouteViewController?
    {
    ...
    func isContainSequence(item: LinkItem) -> Bool {

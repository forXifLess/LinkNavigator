...
extension SingleLinkNavigator {
    ...
    private func _popToRoot(isAnimated: Bool) {
        rootController?.popToRootViewController(animated: isAnimated)
    }
}

// MARK: - Main

public extension SingleLinkNavigator {
...
extension SingleLinkNavigator: LinkNavigatorProtocol {
    ...
    public func popToRoot(isAnimated: Bool) {
        _popToRoot(isAnimated: isAnimated)
    }
...
private extension UINavigationController {
    ...
    func currentItemList() -> [String] {
        viewControllers.compactMap { $0 as? MatchPathUsable }.map(\.matchPath)
    }

    func merge(new: [UIViewController], isAnimated: Bool) {
    ...
    func clear(isAnimated: Bool) {
        setViewControllers([], animated: isAnimated)
    }

    func replace(viewController: [UIViewController], isAnimated: Bool) {
        setViewControllers(viewController, animated: isAnimated)
    }

    func popTo(viewController: UIViewController?, isAnimated: Bool) {
        guard let viewController else { return }
        popToViewController(viewController, animated: isAnimated)
    }

    func dropLast() -> [UIViewController] {
